...
// placeholders will be replaced by CI
const VISITOR_API_URL = "__VISITOR_API_URL__";
const CONTACT_API_URL = "__CONTACT_API_URL__";

async function logVisitor() {
  if (!VISITOR_API_URL || VISITOR_API_URL.includes("__VISITOR_API_URL__")) return;
  try {
...
async function handleFormSubmission(e) {
  ...
  if (!referralCode) { alert("Referral code is required."); return; }
  if (!name || !email || !message) { alert("Please fill in all fields."); return; }
  if (!CONTACT_API_URL || CONTACT_API_URL.includes("__CONTACT_API_URL__")) {
    alert("Contact endpoint not configured yet. Try again later."); return;
  }
  ...
  try {
    ...
    if (response.ok) { alert(result.message || "Message sent successfully!"); document.getElementById("contactForm").reset(); }
